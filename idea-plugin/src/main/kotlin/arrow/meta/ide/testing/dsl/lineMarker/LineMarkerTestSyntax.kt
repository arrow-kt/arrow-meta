 ...
 */
data class LineMarkerDescription(val lineMarker: List<LineMarkerInfo<*>> = emptyList(), val slowLM: List<LineMarkerInfo<*>> = emptyList())
...
interface LineMarkerTestSyntax {
   ...
   */
  fun List<PsiElement>.collectLM(icon: Icon): List<LineMarkerInfo<*>> =
   ...
   */
  fun List<PsiElement>.collectSlowLM(icon: Icon): List<LineMarkerInfo<*>> {
    val r = mutableListOf<LineMarkerInfo<*>>()
    LineMarkerProviders.getInstance().allForLanguage(KotlinLanguage.INSTANCE)
