          ...
          driver = ToolComponent(
            ...
            rules = errors.map {
              ReportingDescriptor(
                ...
                shortDescription = MultiformatMessageString(text = it.errorsId.shortDescription),
...
private fun ReportedError.toResult() = io.github.detekt.sarif4k.Result(
  ruleID = "arrow.analysis.$id",
  level = severity.toResultLevel(),
  locations = (listOf(element.location()) + references.map { it.location() }).mapNotNull { it?.toLocation() }.toSet()
    .toList(),
  message = Message(text = msg)
)

private fun CompilerMessageSourceLocation.toLocation() = Location(
