...
class KotlinResolutionContext(
  ...
  override fun type(typeReference: TypeReference?): Type? =
    typeReference?.let {
      (it.psiOrParent as? KtExpression)
        ?.let { expr -> bindingContext.getType(expr) }
    }?.let { KotlinType(it) }
  ...
  private fun descriptorWorker(fqName: FqName): List<org.jetbrains.kotlin.descriptors.DeclarationDescriptor> {
    ...
    for (portion in portions.drop(1)) {
      current = when (portion) {
        "<init>" -> current.flatMap {
          ...
          when (decl) {
            is org.jetbrains.kotlin.descriptors.PackageViewDescriptor -> decl.memberScope.getContributedDescriptors { true }
            is org.jetbrains.kotlin.descriptors.ClassDescriptor -> decl.getAllDescriptors()
            is org.jetbrains.kotlin.descriptors.TypeAliasDescriptor -> decl.classDescriptor?.getAllDescriptors()
            else -> null
          }.orEmpty().flatMap {
            when (it) {
              is org.jetbrains.kotlin.descriptors.PropertyDescriptor -> listOfNotNull(it, it.getter, it.setter)
              else -> listOf(it)
            }
  ...
  override fun backingPropertyForConstructorParameter(parameter: ValueParameterDescriptor): PropertyDescriptor? =
