...
object ResultLaws {
  ...
  inline fun <T> Result<T>.getOrNullLaw(): T? =
    this.getOrNull().post({ result -> (this.isSuccess) xor (result == null) }) { "null if failure" }

  @Law
  inline fun <T> Result.Companion.successLaw(x: T): Result<T> =
    success(x).post({ it.isSuccess == true }) { "create a success" }

  @Law
  inline fun <T> Result.Companion.failureLaw(e: Throwable): Result<T> =
