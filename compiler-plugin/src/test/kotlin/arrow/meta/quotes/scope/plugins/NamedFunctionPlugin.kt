...
val Meta.namedFunctionDescriptor: CliPlugin
  get() = "Transform Replace Function Descriptor" {
    meta(
      namedFunction(this, { descriptor?.returnType.subtypeOf("CharSequence") && element.name == "descriptorEvaluation" }) { (f, descriptor) ->
        Transform.replace(
          replacing = f,
          newDeclaration = """
