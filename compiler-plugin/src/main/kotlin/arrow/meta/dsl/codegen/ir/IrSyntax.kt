...
interface IrSyntax {
   ...
   * [IR Example]
   */
  fun IrGeneration(generate: (compilerContext: CompilerContext, irModuleFragment: IrModuleFragment, pluginContext: IrPluginContext) -> Unit): IRGeneration =
    object : IRGeneration {
      override fun CompilerContext.generate(
        irModuleFragment: IrModuleFragment,
        pluginContext: IrPluginContext
      ) {
        module = pluginContext.moduleDescriptor
        generate(this, irModuleFragment, pluginContext)
      }
    }

  fun irElement(f: IrUtils.(IrElement) -> IrElement?): IRGeneration =
    IrGeneration { compilerContext, irModuleFragment, pluginContext ->
      irModuleFragment.transformChildren(object : IrElementTransformer<Unit> {
  ...
  fun irDump(): IRGeneration = IrGeneration { compilerContext, irModuleFragment, pluginContext ->
