...
open class ObjectDeclarationPlugin : Meta {
  override fun intercept(ctx: CompilerContext): List<CliPlugin> = listOf(objectDeclarationPlugin)
}
  ...
  get() = "Object Declaration Scope Plugin" {
    meta(
      objectDeclaration(this, { element.name == "Test" }) {
        Transform.replace(replacing = it.element, newDeclaration = identity())
      }
    )
  }
