...
open class ClassBodyPlugin : Meta {
  override fun intercept(ctx: CompilerContext): List<CliPlugin> = listOf(classBody)
}

// TODO remove after splitting elements apart
open class EnumBodyPlugin : Meta {
  override fun intercept(ctx: CompilerContext): List<CliPlugin> = listOf(enumBody)
}

private val Meta.classBody: CliPlugin
  get() = "Class Body Scope Plugin" {
    meta(
