...
/**
 * The property scope used in destructuring templates & the
 * [arrow.meta.phases.analysis.ElementScope] DSL
 */
open class Scope<out K : KtElement>(open val value: K?) {
  ...
  override fun toString(): String = if (value != null) value?.text ?: "" else "" //
...
fun <K : KtElement> Scope<K>?.orEmpty(): Scope<K> = this ?: Scope.empty<K>()

fun <K : KtElement> K?.scope(): Scope<K> = Scope(this)
