...
interface InternalRegistry : ConfigSyntax {
  ...
  fun registerMetaComponents(
    ...
    cli { registerSyntheticScopeProviderIfNeeded(project) }
        ...
        fun ExtensionPhase.registerPhase(): Unit {
          when (this) {
            ...
            is PreprocessedVirtualFileFactory ->
              registerPreprocessedVirtualFileFactory(project, this, ctx)
            ...
            is DeclarationAttributeAlterer ->
              registerDeclarationAttributeAlterer(project, this, ctx)
            ...
            is FirMetaExtension -> registerFirMetaExtension(project, this, ctx)
  ...
  fun registerPreprocessedVirtualFileFactory(
    project: Project,
    ...
    KTypeResolutionInterceptor.registerExtension(
      ...
      object : KTypeResolutionInterceptorExtension {
        override fun interceptFunctionLiteralDescriptor(
          ...
          phase.run { ctx.interceptFunctionLiteralDescriptor(expression, context, descriptor) }

        override fun interceptType(
          element: KtElement,
          context: ExpressionTypingContext,
          resultType: KotlinType
        ): KotlinType = phase.run { ctx.interceptType(element, context, resultType) }
      }
    )
  }
  ...
  fun packageFragmentProvider(project: Project, phase: PackageProvider, ctx: CompilerContext) {
  ...
  fun registerAnalysisHandler(project: Project, phase: AnalysisHandler, ctx: CompilerContext) {
  ...
  fun registerClassBuilder(project: Project, phase: ClassBuilder, ctx: CompilerContext) {
  ...
  fun registerCompilerConfiguration(project: Project, phase: Config, ctx: CompilerContext) {
  ...
  fun registerFirMetaExtension(project: Project, phase: FirMetaExtension, ctx: CompilerContext) {
    FirExtensionRegistrar.registerExtension(
      project,
      object : FirExtensionRegistrar() {
        override fun ExtensionRegistrarContext.configurePlugin() {
          phase.run {
            +{ firSession: FirSession -> ctx.configureFirAdditionalCheckersExtension(firSession) }
            +{ firSession: FirSession -> ctx.configureFirDeclarationGenerationExtension(firSession) }
            +{ firSession: FirSession -> ctx.configureFirStatusTransformerExtension(firSession) }
            +{ firSession: FirSession -> ctx.configureFirSupertypeGenerationExtension(firSession) }
            +{ firSession: FirSession -> ctx.configureFirTypeAttributeExtension(firSession) }
          }
        }
      }
    )
  ...
  fun compilerContextService(): StorageComponentContainer =
    storageComponent(
      registerModuleComponents = { container, moduleDescriptor -> container.useInstance(this) },
