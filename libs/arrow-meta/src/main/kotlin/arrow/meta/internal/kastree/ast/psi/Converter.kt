...
open class Converter {
  protected open fun onNode(node: Node, elem: PsiElement) {
    node.psiElement = elem
  }

  open fun convertAnnotated(v: KtAnnotatedExpression) =
    Node.Expr.Annotated(
      ...
      .map(v)
      .let {
        // As a special case, instead of annotating a type/binary op, we mean to just annotate its
        // lhs
        val expr = it.expr
        when (expr) {
          is Node.Expr.BinaryOp -> expr.copy(lhs = it.copy(expr = expr.lhs))
          is Node.Expr.TypeOp -> expr.copy(lhs = it.copy(expr = expr.lhs))
          else -> it
        }
      }

  open fun convertAnnotation(v: KtAnnotationEntry) =
  ...
  open fun convertAnnotationSet(v: KtAnnotation) =
  ...
  open fun convertAnnotationSets(v: KtElement): List<Node.Modifier.AnnotationSet> =
    v.children.flatMap { elem ->
      ...
      when (elem) {
        ...
        is KtAnnotation -> listOf(convertAnnotationSet(elem))
        is KtFileAnnotationList -> convertAnnotationSets(elem)
        else -> emptyList()
      }
    }
  ...
  open fun convertArrayAccess(v: KtArrayAccessExpression) =
  ...
  open fun convertBinaryOp(v: KtBinaryExpression) =
    Node.Expr.BinaryOp(
        lhs = convertExpr(v.left ?: error("No binary lhs for $v")),
        oper =
          binaryTokensByText[v.operationReference.text].let {
  ...
  open fun convertBinaryOp(v: KtQualifiedExpression) =
    Node.Expr.BinaryOp(
        lhs = convertExpr(v.receiverExpression),
        oper =
          Node.Expr.BinaryOp.Oper.Token(
            if (v is KtDotQualifiedExpression) Node.Expr.BinaryOp.Token.DOT
            else Node.Expr.BinaryOp.Token.DOT_SAFE
          ),
        rhs = convertExpr(v.selectorExpression ?: error("No qualified rhs for $v"))
      )
      .map(v)

  open fun convertBlock(v: KtBlockExpression) =
    Node.Block(stmts = v.statements.map(::convertStmtNo)).map(v)

  open fun convertBrace(v: KtBlockExpression) =
    Node.Expr.Brace(params = emptyList(), block = convertBlock(v)).map(v)

  open fun convertBrace(v: KtFunctionLiteral) =
    Node.Expr.Brace(
  ...
  open fun convertBrace(v: KtLambdaExpression) =
    Node.Expr.Brace(
        params = v.valueParameters.map(::convertBraceParam),
        block = v.bodyExpression?.let(::convertBlock)
      )
  ...
  open fun convertBraceParam(v: KtParameter) =
    Node.Expr.Brace.Param(
        vars = convertPropertyVars(v),
        destructType =
          if (v.destructuringDeclaration != null) v.typeReference?.let(::convertType) else null
      )
      .map(v)

  open fun convertBreak(v: KtBreakExpression) = Node.Expr.Break(label = v.getLabelName()).map(v)

  open fun convertCall(v: KtCallExpression) =
    Node.Expr.Call(
    ...
    fun KtExpression.extractLambda(allowParens: Boolean = false): KtLambdaExpression? =
      when (this) {
        ...
        is KtLabeledExpression ->
          baseExpression?.extractLambda(allowParens).also { label = getLabelName() }
        is KtAnnotatedExpression ->
          baseExpression?.extractLambda(allowParens).also { anns = convertAnnotationSets(this) }
        is KtParenthesizedExpression ->
          if (allowParens) expression?.extractLambda(allowParens) else null
        else -> null
      }
    val expr = v.getArgumentExpression()?.extractLambda() ?: error("No lambda for $v")
    return Node.Expr.Call.TrailLambda(anns = anns, label = label, func = convertBrace(expr)).map(v)
  }

  open fun convertCollLit(v: KtCollectionLiteralExpression) =
    Node.Expr.CollLit(exprs = v.getInnerExpressions().map(::convertExpr)).map(v)

  open fun convertConst(v: KtConstantExpression) =
    Node.Expr.Const(
      value = v.text,
      form =
  ...
  open fun convertConstructor(v: KtSecondaryConstructor) =
    Node.Decl.Constructor(
        mods = convertModifiers(v),
        params = v.valueParameters.map(::convertFuncParam),
        delegationCall =
          if (v.hasImplicitDelegationCall()) null
          else
  ...
  open fun convertContinue(v: KtContinueExpression) =
    Node.Expr.Continue(label = v.getLabelName()).map(v)

  open fun convertDecl(v: KtDeclaration): Node.Decl =
    when (v) {
  ...
  open fun convertEnumEntry(v: KtEnumEntry) =
    Node.Decl.EnumEntry(
        mods = convertModifiers(v),
        name = v.name ?: error("Unnamed enum"),
        args =
          convertValueArgs(
            (v.superTypeListEntries.firstOrNull() as? KtSuperTypeCallEntry)?.valueArgumentList
          ),
        members = v.declarations.map(::convertDecl)
      )
      .map(v)

  open fun convertExpr(v: KtExpression): Node.Expr =
    when (v) {
  ...
  open fun convertFile(v: KtFile, sourcePath: FqName? = null) =
    Node.File(
        anns = convertAnnotationSets(v),
  ...
  open fun convertFor(v: KtForExpression) =
    Node.Expr.For(
  ...
  open fun convertFunc(v: KtNamedFunction) =
    Node.Decl.Func(
        mods = convertModifiers(v),
        typeParams =
          if (v.hasTypeParameterListBeforeFunctionName()) v.typeParameters.map(::convertTypeParam)
          else emptyList(),
  ...
  open fun convertFuncParam(v: KtParameter) =
  ...
  open fun convertIf(v: KtIfExpression) =
    Node.Expr.If(
  ...
  open fun convertImportDirective(v: KtImportDirective) =
    Node.Import(
  ...
  open fun convertInit(v: KtAnonymousInitializer) =
    Node.Decl.Init(
        block = convertBlock(v.body as? KtBlockExpression ?: error("No init block for $v"))
      )
      .map(v)

  open fun convertLabeled(v: KtLabeledExpression) =
    Node.Expr.Labeled(
            ...
            is PsiWhiteSpace -> null
              ...
              when (node.text) {
                ...
                "value" -> null
                  ...
                  modifiersByText[node.text]?.let {
                    Node.Modifier.Lit(it).let { lit ->
                      (node.psi as? KtElement)?.let { lit.map(it) } ?: lit
                    }
                  }
  ...
  open fun convertName(v: KtSimpleNameExpression) =
    Node.Expr.Name(name = v.getReferencedName()).map(v)
  ...
  open fun convertPackage(v: KtPackageDirective) =
    Node.Package(mods = convertModifiers(v), names = v.packageNames.map { it.getReferencedName() })
      .map(v)

  open fun convertParen(v: KtParenthesizedExpression) =
    Node.Expr.Paren(expr = convertExpr(v.expression ?: error("No paren expr for $v")))

  open fun convertParent(v: KtSuperTypeListEntry) =
    when (v) {
      is KtSuperTypeCallEntry ->
        ...
        Node.Decl.Structured.Parent.Type(
            type = v.typeReference?.let(::convertTypeRef) as? Node.TypeRef.Simple
                ?: error("Bad type on super call $v"),
  ...
  open fun convertProperty(v: KtDestructuringDeclaration) =
    Node.Decl.Property(
        mods = convertModifiers(v),
        readOnly = !v.isVar,
  ...
  open fun convertProperty(v: KtProperty) =
    Node.Decl.Property(
        mods = convertModifiers(v),
        readOnly = !v.isVar,
        ...
        accessors =
          v.accessors.map(::convertPropertyAccessor).let {
            if (it.isEmpty()) null
            else Node.Decl.Property.Accessors(first = it.first(), second = it.getOrNull(1))
          }
      )
      .map(v)
  ...
  open fun convertPropertyExpr(v: KtDestructuringDeclaration) =
    Node.Expr.Property(decl = convertProperty(v)).map(v)

  open fun convertPropertyExpr(v: KtProperty) = Node.Expr.Property(decl = convertProperty(v)).map(v)
  ...
  open fun convertPropertyVars(v: KtParameter) =
    v.destructuringDeclaration?.entries?.map(::convertPropertyVar)
      ?: listOf(
        if (v.name == "_") null
        else
          Node.Decl.Property.Var(
  ...
  open fun convertReturn(v: KtReturnExpression) =
    Node.Expr.Return(label = v.getLabelName(), expr = v.returnedExpression?.let(::convertExpr))
      .map(v)

  open fun convertStmtNo(v: KtExpression) =
    if (v is KtDeclaration) Node.Stmt.Decl(convertDecl(v)).map(v)
    else Node.Stmt.Expr(convertExpr(v)).map(v)

  open fun convertStringTmpl(v: KtStringTemplateExpression) =
  ...
  open fun convertStringTmplElem(v: KtStringTemplateEntry) =
    when (v) {
      is KtLiteralStringTemplateEntry -> Node.Expr.StringTmpl.Elem.Regular(v.text).map(v)
      is KtSimpleNameStringTemplateEntry ->
        Node.Expr.StringTmpl.Elem.ShortTmpl(v.expression?.text ?: error("No short tmpl text"))
          .map(v)
      is KtBlockStringTemplateEntry ->
        Node.Expr.StringTmpl.Elem.LongTmpl(convertExpr(v.expression ?: error("No expr tmpl")))
          .map(v)
      is KtEscapeStringTemplateEntry ->
        if (v.text.startsWith("\\u"))
          Node.Expr.StringTmpl.Elem.UnicodeEsc(v.text.substring(2)).map(v)
        else Node.Expr.StringTmpl.Elem.RegularEsc(v.unescapedValue.first()).map(v)
      else -> error("Unrecognized string template type for $v")
    }

  open fun convertStructured(v: KtClassOrObject) =
  ...
  open fun convertSuper(v: KtSuperExpression) =
    Node.Expr.Super(typeArg = v.superTypeQualifier?.let(::convertType), label = v.getLabelName())
      .map(v)

  open fun convertThis(v: KtThisExpression) = Node.Expr.This(label = v.getLabelName()).map(v)

  open fun convertThrow(v: KtThrowExpression) =
    Node.Expr.Throw(expr = convertExpr(v.thrownExpression ?: error("No throw expr for $v"))).map(v)

  open fun convertTry(v: KtTryExpression) =
    Node.Expr.Try(
  ...
  open fun convertTryCatch(v: KtCatchClause) =
    Node.Expr.Try.Catch(
  ...
  open fun convertType(v: KtTypeReference, modifierList: KtModifierList?): Node.Type =
    Node.Type(mods = convertModifiers(modifierList), ref = convertTypeRef(v)).map(v)
  ...
  open fun convertTypeAlias(v: KtTypeAlias) =
    Node.Decl.TypeAlias(
        mods = convertModifiers(v),
  ...
  open fun convertTypeConstraint(v: KtTypeConstraint) =
    Node.TypeConstraint(
        anns =
          v.children.mapNotNull {
            when (it) {
              is KtAnnotationEntry ->
                Node.Modifier.AnnotationSet(target = null, anns = listOf(convertAnnotation(it)))
                  .map(it)
              is KtAnnotation -> convertAnnotationSet(it)
              else -> null
            }
  ...
  open fun convertTypeOp(v: KtBinaryExpressionWithTypeRHS) =
    Node.Expr.TypeOp(
        lhs = convertExpr(v.left),
        oper =
          v.operationReference.let {
  ...
  open fun convertTypeOp(v: KtIsExpression) =
    Node.Expr.TypeOp(
  ...
  open fun convertTypeParam(v: KtTypeParameter) =
    Node.TypeParam(
        mods = convertModifiers(v),
  ...
  open fun convertTypeParams(v: KtTypeArgumentList?) =
    v?.arguments?.map { if (it.projectionKind == KtProjectionKind.STAR) null else convertType(it) }
      ?: emptyList()

  open fun convertTypeRef(v: KtTypeReference) =
    convertTypeRef(v.typeElement ?: error("Missing typ elem")).let {
      if (!v.hasParentheses()) it
      else Node.TypeRef.Paren(mods = convertModifiers(v), type = it).map(v)
    }

  open fun convertTypeRef(v: KtTypeElement): Node.TypeRef =
    when (v) {
      is KtFunctionType ->
        Node.TypeRef.Func(
            receiverType = v.receiverTypeReference?.let(::convertType),
      ...
      is KtUserType ->
        Node.TypeRef.Simple(
            pieces =
              generateSequence(v) { it.qualifier }.toList().reversed().map {
                Node.TypeRef.Simple.Piece(
      ...
      is KtNullableType ->
        Node.TypeRef.Nullable(
            ...
            type =
              convertModifiers(v.modifierList).let { mods ->
  ...
  open fun convertUnaryOp(v: KtUnaryExpression) =
    Node.Expr.UnaryOp(
  ...
  open fun convertValueArg(v: KtValueArgument) =
    Node.ValueArg(
  ...
  open fun convertWhen(v: KtWhenExpression) =
    Node.Expr.When(
  ...
  open fun convertWhenEntry(v: KtWhenEntry) =
  ...
  open fun convertWhile(v: KtWhileExpressionBase) =
    Node.Expr.While(
  ...
  open fun convertClassBody(v: KtClassBody) =
    Node.Decl.ClassBody(members = v.declarations.map(::convertDecl))
  ...
  open class WithExtras : Converter(), ExtrasMap {
    // Sometimes many nodes are created from the same element, but we only want the last node we're
    // given. We
    // remove the previous nodes we've found for the same identity when we see a new one. So we
    // don't have to
    // keep PSI elems around, we hold a map to the element's identity hash code. Then we use that
    // number to tie
    // to the extras to keep duplicates out. Usually using identity hash codes would be problematic
    // due to
    ...
    internal val allExtrasBefore
      get() = extrasBefore
    internal val allExtrasAfter
      get() = extrasAfter
    ...
    override fun onNode(node: Node, elem: PsiElement) {
      ...
      convertExtras(beforeElems)
        .map {
          // As a special case, we make sure all non-block comments start a line when "before"
          if (it is Node.Extra.Comment && !it.startsLine && it.text.startsWith("//"))
            it.copy(startsLine = true)
          else it
        }
    ...
    open fun nodeExtraElems(
      elem: PsiElement
    ): Triple<List<PsiElement>, List<PsiElement>, List<PsiElement>> {
      ...
      // After includes all siblings before the first newline or all if there are only ws/comment
      // siblings
    ...
    open fun convertExtras(elems: List<PsiElement>): List<Node.Extra> =
      elems.mapNotNull { elem ->
        ...
        val elemId = System.identityHashCode(elem)
        if (!seenExtraPsiIdentities.add(elemId)) null
        else
          when (elem) {
            is PsiWhiteSpace ->
              elem.text.count { it == '\n' }.let { newlineCount ->
            ...
            is PsiComment ->
              Node.Extra.Comment(
                  text = elem.text,
                  startsLine =
                    ((elem.prevSibling ?: elem.prevLeaf()) as? PsiWhiteSpace)?.textContains('\n') ==
                      true,
                  endsLine =
                    elem.tokenType == KtTokens.EOL_COMMENT ||
