...
open class Writer(
    ...
    v?.apply {
      ...
      if (replacedSource != null) append(replacedSource)
        ...
        when (this) {
          ...
          is Node.Package -> childMods().append("package ").appendNames(names, ".")
          is Node.Import -> {
          ...
          is Node.Decl.Structured ->
            childMods().also {
              ...
              // As a special case, if an object is nameless and bodyless, we should give it an
              // empty body
          ...
          is Node.Decl.Structured.PrimaryConstructor -> {
            if (mods.isNotEmpty())
              append(" ").also { childMods(newlines = false).append("constructor") }
            parenChildren(params)
          }
          is Node.Decl.Init -> append("init ").also { children(block) }
          is Node.Decl.Func -> {
          ...
          is Node.Decl.Func.Body.Block -> children(block)
          is Node.Decl.Func.Body.Expr -> append("= ").also { children(expr) }
          ...
          is Node.Decl.EnumEntry -> {
            ...
            if (members.isNotEmpty())
              lineEnd(" {").indented { childrenLines(members, extraMidLines = 1) }.lineBegin("}")
          }
          ...
          is Node.TypeRef.Paren ->
            append('(').also { childModsBeforeType(type).also { children(type) } }.append(')')
          ...
          is Node.TypeRef.Simple -> children(pieces, ".")
          is Node.TypeRef.Simple.Piece -> appendName(name).also { bracketedChildren(typeParams) }
          is Node.TypeRef.Nullable -> children(type).append('?')
          is Node.TypeRef.Dynamic -> append("dynamic")
          is Node.Type -> childModsBeforeType(ref).also { children(ref) }
          is Node.ValueArg -> {
            if (name != null) appendName(name).append(" = ")
            if (asterisk) append('*')
            children(expr)
          ...
          is Node.Expr.While -> {
            if (!doWhile)
              append("while (").also { children(expr) }.append(") ").also { children(body) }
            else
              append("do ")
                .also { children(body) }
                .append(" while (")
          ...
          is Node.Expr.BinaryOp -> {
            ...
            val noSep =
              oper is Node.Expr.BinaryOp.Oper.Token &&
                oper.token.let {
                  it == Node.Expr.BinaryOp.Token.RANGE ||
                    it == Node.Expr.BinaryOp.Token.DOT ||
                    it == Node.Expr.BinaryOp.Token.DOT_SAFE
                }
            children(listOf(lhs, oper, rhs), if (noSep) "" else " ")
          }
          is Node.Expr.BinaryOp.Oper.Infix -> append(str)
          is Node.Expr.BinaryOp.Oper.Token -> append(token.str)
          is Node.Expr.UnaryOp -> if (prefix) children(oper, expr) else children(expr, oper)
          is Node.Expr.UnaryOp.Oper -> append(token.str)
          is Node.Expr.TypeOp -> children(listOf(lhs, oper, rhs), " ")
          is Node.Expr.TypeOp.Oper -> append(token.str)
          ...
          is Node.Expr.DoubleColonRef.Recv.Expr -> children(expr)
          is Node.Expr.DoubleColonRef.Recv.Type -> children(type).append("?".repeat(questionMarks))
          is Node.Expr.Paren -> append('(').also { children(expr) }.append(')')
          is Node.Expr.StringTmpl ->
            if (raw) append("\"\"\"").also { children(elems) }.append("\"\"\"")
            else append('"').also { children(elems) }.append('"')
          is Node.Expr.StringTmpl.Elem.Regular -> append(str)
          is Node.Expr.StringTmpl.Elem.ShortTmpl -> append('$').appendName(str)
          is Node.Expr.StringTmpl.Elem.UnicodeEsc -> append("\\u").append(digits)
          ...
          is Node.Expr.StringTmpl.Elem.LongTmpl -> append("\${").also { children(expr) }.append('}')
          is Node.Expr.Const -> append(value)
          is Node.Expr.Brace -> {
            append('{')
          ...
          is Node.Expr.When.Entry -> {
            if (conds.isEmpty()) append("else") else children(conds, ", ")
            append(" -> ").also { children(body) }
          }
          is Node.Expr.When.Cond.Expr -> children(expr)
          ...
          is Node.Expr.Object -> {
            ...
            if (members.isEmpty()) append(" {}")
            else lineEnd(" {").indented { childrenLines(members, extraMidLines = 1) }.lineBegin("}")
          }
          is Node.Expr.Throw -> append("throw ").also { children(expr) }
          is Node.Expr.Return -> {
            append("return")
            if (label != null) append('@').appendName(label)
          ...
          is Node.Expr.CollLit -> children(exprs, ", ", "[", "]")
          is Node.Expr.Name -> appendName(name)
          is Node.Expr.Labeled -> appendName(label).append("@ ").also { children(expr) }
          is Node.Expr.Annotated -> childAnnsBeforeExpr(expr).also { children(expr) }
          is Node.Expr.Call -> {
            children(expr)
          ...
          is Node.Expr.AnonFunc -> children(func)
          is Node.Expr.Property -> children(decl)
          is Node.Block -> {
          ...
          is Node.Stmt.Expr -> children(expr)
          ...
          is Node.Modifier.Lit -> append(keyword.name.toLowerCase())
          else -> error("Unrecognized node type: $this")
        }
    }
    v?.writeExtrasAfter()
  ...
  protected open fun Node.writeExtrasAfter() {
    ...
    writeExtras(
      extrasMap.extrasAfter(this).takeWhile {
  ...
  protected open fun Node.writeExtrasLineEnd() {
    ...
    writeExtras(
      extrasMap.extrasAfter(this).dropWhile {
        it is Node.Extra.Comment && !it.startsLine && !it.endsLine
      }
    )
  }
    ...
    extras.forEach {
      when (it) {
        ...
        is Node.Extra.Comment -> {
          if (it.startsLine && it.endsLine) lineEnd(it.text).lineBegin()
          else {
  ...
  protected fun Node.childTypeConstraints(v: List<Node.TypeConstraint>) =
    this@Writer.also { if (v.isNotEmpty()) append(" where ").also { children(v, ", ") } }
  ...
  protected fun Node.WithModifiers.childModsBeforeType(ref: Node.TypeRef) =
    this@Writer.also {
      if (mods.isNotEmpty()) {
        // As a special case, if there is a trailing annotation with no args and the ref has a paren
        // which is a paren
        // type or a non-receiver fn type, then we need to add an empty set of parens ourselves
        val lastAnn =
          (mods.lastOrNull() as? Node.Modifier.AnnotationSet)?.anns?.singleOrNull()?.takeIf {
            it.args.isEmpty()
          }
        val shouldAddParens =
          lastAnn != null &&
  ...
  protected fun stmtRequiresEmptyBraceSetBeforeLineEnd(v: Node?, next: Node?): Boolean {
    ...
    if (v !is Node.Stmt.Decl ||
        ...
        v.decl.members?.members?.isNotEmpty() == true ||
  ...
  // We accept lots of false positives to be simple and not have to bring in JVM dep to do accurate
  // check
