...
/**
 * The [cli] function selectively evaluates [f] in the Command Line Compiler.
 * [f] is ignored if this is an IDEA plugin
 */
fun <A> cli(f: () -> A): A? =
  if (isCli) f() else null

/**
 * The [ide] function selectively evaluates [f] in the IDEA plugin.
 * [f] is ignored if this is an command line compiler plugin
 */
fun <A> ide(f: () -> A): A? =
