...
internal class Wrapper(val value: Any, override val irClass: IrClass) : Complex(irClass, mutableListOf()) {
  ...
  fun getMethod(irFunction: IrFunction): MethodHandle? {
    if (irFunction.getEvaluateIntrinsicValue()
        ?.isEmpty() == true
    ) return null // this method will handle IntrinsicEvaluator
  ...
  companion object {
    ...
    fun getCompanionObject(irClass: IrClass): Wrapper {
      ...
      val objectValue =
        companionObjectValue[objectName] ?: throw AssertionError("Companion object $objectName cannot be interpreted")
